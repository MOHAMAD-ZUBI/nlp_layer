#!/usr/bin/env python3
"""
Interactive script to test prompts against the prompt injection classifier.
"""
...
def predict(model, tokenizer, device, prompt):
    """Predict if a prompt is an injection attempt."""
    inputs = tokenizer(
        prompt,
        padding=True,
        truncation=True,
...
def print_result(prompt, result):
    ...
    print(f"  INJECTION: {result['injection_prob']:6.2f}%")
    print("-" * 60)
